...
for overlay_file in ${overlays}; do
        if load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}dtbs/allwinner/overlay/${overlay_prefix}-${overlay_file}.dtbo; then
                echo "Applying kernel provided DT overlay ${overlay_prefix}-${overlay_file}.dtbo"
                fdt apply ${load_addr} || setenv overlay_error "true"
        elif load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}dtbs/allwinner/overlay/${overlay_file}.dtbo; then
...
if test "${overlay_error}" = "true"; then
        ...
        if test -e ${devtype} ${devnum}:${distro_bootpart} ${prefix}dtbs/allwinner/overlay/${overlay_prefix}-fixup.scr; then
                load ${devtype} ${devnum}:${distro_bootpart} ${load_addr} ${prefix}dtbs/allwinner/overlay/${overlay_prefix}-fixup.scr

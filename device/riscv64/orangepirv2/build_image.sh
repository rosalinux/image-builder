...
echo "[+] Calculating size of $IMAGE_DIR..."
DIR_MB=$(du -sm "$IMAGE_DIR" | cut -f1)
TOTAL_MB=$((DIR_MB + MIN_EXTRA_MB))
echo "[+] Target image size: $TOTAL_MB MB"

# === Step 3: Create empty image file ===
echo "[+] Creating image file $IMAGE_NAME..."
...
echo "[+] Creating DOS (MBR) partition table..."
...
for i in {1..10}; do
    ...
    echo "[!] Waiting for partition device $PARTITION_DEV to appear..."
    sleep 0.5
done

if [[ ! -b "$PARTITION_DEV" ]]; then
    echo "Partition device $PARTITION_DEV not found."
    losetup -d "$LOOP_DEV"
    exit 1
fi

# === Step 7: Format partition as ext4 ===
echo "[+] Formatting $PARTITION_DEV as ext4..."
...
echo "[+] Copying files to mounted partition..."
...
if [ -f "$BOOTINFO_SD" ]; then
    echo "[+] Writing bootloader files to ${LOOP_DEV}..."
...
echo "[✓] Image $IMAGE_NAME created, formatted and populated successfully."
